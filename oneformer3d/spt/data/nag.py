...
import oneformer3d.spt
from oneformer3d.spt.data import Data, Batch
from oneformer3d.spt.utils import tensor_idx, has_duplicates, sparse_sample
...
class NAG:
    ...
    def get_sub_size(
        self, high: int, low: int = 0, use_level_zero_node_size: bool = True
    ) -> List[torch.Tensor]:
    ...
    def get_super_index(self, high: int, low: int = 0) -> List[torch.Tensor]:
    ...
    def select(
        self, i_level: int, idx: Union[int, List[int], torch.Tensor, np.ndarray]
    ) -> "NAG":
...
class NAGBatch(NAG):
    ...
    def __init__(self, batch_list: List[Batch]):
        assert all(
            [isinstance(b, Batch) for b in batch_list]
    ...
    def from_nag_list(cls, nag_list: List["NAG"]) -> "NAGBatch":
        ...
        return cls([Batch.from_data_list(l) for l in zip(*[n._list for n in nag_list])])

    def to_nag_list(self, strict: bool = False) -> List["NAG"]:
        return [NAG(l) for l in zip(*[b.to_data_list(strict=strict) for b in self])]
