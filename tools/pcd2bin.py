...
def pcd_to_bin(input_pcd, output_bin):
    ...
    # print(f"point cloud loaded from '{input_pcd}'")
    ...
    if colors.size == 0:
        # print("Warning: no color information found, using default white color")
...
if __name__ == "__main__":
    ...
    if osp.isdir(args.input_pcd ):

        for file in os.listdir(args.input_pcd):
            if file.endswith(".pcd"):
                pcd_to_bin(osp.join(args.input_pcd, file), osp.join(args.output_bin, file.replace(".pcd", ".bin")))
    else:
